-- =====================================================
-- BORRAR TODO Y CREAR DATOS CON SENTIDO
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE 'Base de datos limpiada';
  ...
  -- GGPoker: Action 60%, Diamond→Club Tabla, Diamond→Player Tabla
  UPDATE public.clubs
  SET
  ...
  -- PokerStars: Action 55%, Diamond→Club Fijo 50%, Diamond→Player Tabla
  UPDATE public.clubs
  SET
    action_percentage = 55.0,
  ...
  -- 888poker: Action 50%, Diamond→Club Fijo 45%, Diamond→Player Fijo 30%
  UPDATE public.clubs
  SET
    action_percentage = 50.0,
    diamond_club_agreement_type = 'fixed',
    diamond_club_fixed_percentage = 45.0,
    diamond_player_agreement_type = 'fixed',
    diamond_player_fixed_percentage = 30.0
  WHERE code = '888POKER';

  -- PartyPoker: Action 58%, Diamond→Club Tabla, Diamond→Player Fijo 25%
  UPDATE public.clubs
  SET
    action_percentage = 58.0,
    diamond_club_agreement_type = 'dynamic',
    diamond_club_template_id = v_club_template_id,
    diamond_player_agreement_type = 'fixed',
    diamond_player_fixed_percentage = 25.0
  WHERE code = 'PARTYPOKER';

  -- =====================================================
  -- CREAR JUGADORES
  -- =====================================================

  -- 1. Carlos - Normal, sin agente
  ...
  -- 4. Laura - Referida por Ana, con acuerdo FIJO personalizado
  INSERT INTO public.players (player_code, player_id, full_name, nickname, phone, email, status, token_used, is_agent, referred_by_agent_id)
  ...
  -- Carlos en GGPoker (sin agente, usa acuerdos del club)
  ...
  -- Ana en GGPoker (agente, sin comisión)
  INSERT INTO public.player_clubs (player_id, club_id, username_in_club, agent_commission_percentage)
  ...
  -- Laura en PokerStars (referida de Ana, comisión 25%, ACUERDO PERSONALIZADO FIJO 35%)
  ...
  -- Carlos: Ganando, ratio 1.39, 8500 manos
  ...
  -- Ana: Perdiendo poco, ratio -0.33, 4200 manos
  INSERT INTO public.weekly_player_reports (player_club_id, week_start, week_end, pnl, rake, hands)
  VALUES (v_ana_gg_pc_id, v_week_start, v_week_end, -300.00, 900.00, 4200);

  -- Miguel: Ganando, ratio 0.6, 9800 manos
  INSERT INTO public.weekly_player_reports (player_club_id, week_start, week_end, pnl, rake, hands)
  VALUES (v_miguel_gg_pc_id, v_week_start, v_week_end, 1200.00, 2000.00, 9800);

  -- Laura: Perdiendo, rakeback FIJO 35%
  INSERT INTO public.weekly_player_reports (player_club_id, week_start, week_end, pnl, rake, hands)
  VALUES (v_laura_ps_pc_id, v_week_start, v_week_end, -1500.00, 1100.00, 6500);

  RAISE NOTICE 'Datos creados correctamente:';
  RAISE NOTICE '- Carlos: GGPoker (sin agente)';
  RAISE NOTICE '- Ana: GGPoker (AGENTE)';
  RAISE NOTICE '- Miguel: GGPoker (referido Ana 30%%)';
  RAISE NOTICE '- Laura: PokerStars (referida Ana 25%%, rakeback fijo 35%%)';
END $$;
